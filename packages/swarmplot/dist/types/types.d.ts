...
export declare type PreSimulationDatum<RawDatum> = Pick<ComputedDatum<RawDatum>, 'id' | 'group' | 'value' | 'size' | 'data'> & Partial<Pick<ComputedDatum<RawDatum>, 'x' | 'y' | 'index'>>;
export declare type SimulationForces<RawDatum> = {
...
export declare type SwarmPlotLayerId = 'grid' | 'axes' | 'circles' | 'annotations' | 'mesh';
...
export declare type SwarmPlotCustomLayer<RawDatum, Scale extends ScaleLinear<number> | ScaleTime<string | Date> | ScaleOrdinal<string, number> = ScaleLinear<number>> = React.FC<SwarmPlotCustomLayerProps<RawDatum, Scale>>;
export declare type SwarmPlotLayer<RawDatum, Scale extends ScaleLinear<number> | ScaleTime<string | Date> | ScaleOrdinal<string, number> = ScaleLinear<number>> = SwarmPlotLayerId | SwarmPlotCustomLayer<RawDatum, Scale>;
export declare type SizeSpec<RawDatum> = number | ((datum: RawDatum) => number) | {
    key: string;
    values: number[];
    sizes: number[];
};
...
export declare type SwarmPlotCommonProps<RawDatum> = {
    data: RawDatum[];
    width: number;
    height: number;
    margin?: Box;
...
export declare type SwarmPlotSvgProps<RawDatum> = SwarmPlotCommonProps<RawDatum> & MouseHandlers<RawDatum> & {
...
export declare type SwarmPlotCanvasProps<RawDatum> = SwarmPlotCommonProps<RawDatum> & Pick<MouseHandlers<RawDatum>, 'onMouseMove' | 'onClick'> & {
